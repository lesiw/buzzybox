...
func (t *awkTest) run() (stdout, stderr, dir string) {
	var err error
	posix.ResetRandom()
	...
	return outbuf.String(), errbuf.String(), dir
}
...
func TestAwkCliArgs(t *testing.T) {
	tests := []struct {
		...
		in: "hello\ngoodbye\n",
		args: []string{
			"awk", "BEGIN { x=0; print x; getline; print x, $0 }", "x=1",
		},
		out: "0\n1 hello\n",
	}, {
		in: "hello\ngoodbye\n",
		...
		args: []string{
			"awk", "BEGIN { getline; print x }", "x=4", "/dev/null",
		},
		out: "4\n",
	}, {
		...
		args: []string{
			"awk", `BEGIN { x=0; getline; print x } END { print x }`,
			"x=6", "testdata/awk/hello", "x=end",
		},
		out: "6\nend\n",
	}, {
		args: []string{
			"awk", `BEGIN {_=0;getline <"/dev/null";print _} END {print _}`,
		...
		args: []string{
			"awk", "-v", "x=123", "-v", "y=abc", "-v", "z1=10.99", "--",
			`BEGIN { print x, y, z1 }`,
		},
		out: "123 abc 10.99\n",
	}, {
		...
		args: []string{
			"awk", "-v", "x=123", "-v", "y=abc", "-f", "f0", "-v", "z1=10.99",
		},
		out: "123 abc 10.99\n",
	}, {
		infs: []string{"BEGIN { print x, y, z1 }"},
		...
		infs: []string{"BEGIN { print x, y, z1 }"},
		args: []string{
			"awk", "-f", "f0", "-v", "x=123", "-v", "y=abc", "-v", "z1=10.99",
		},
		out: "123 abc 10.99\n",
	}, {
		infs: []string{"BEGIN { print x, y, z1 }"},
		...
		infs: []string{"BEGIN { print x, y, z1 } END { print x }"},
		args: []string{
			"awk", "-f", "f0", "-vx=123", "-vy=abc", "-vz1=10.99", "/dev/null",
			"x=4567", "/dev/null",
		},
		out: "123 abc 10.99\n4567\n",
	}, {
		infs: []string{
			"BEGIN { print x, y, z1 } NR==1 { print x }", "foo\nbar\n",
		},
		args: []string{
			"awk", "-v", "x=123", "-v", "y=abc", "-v", "z1=10.99", "-f", "f0",
			"x=4567", "f1",
		},
		out: "123 abc 10.99\n4567\n",
	}, {
	...
	for _, tt := range tests {
		t.Run(strings.Join(tt.args, " "), func(t *testing.T) {
			...
			if outbuf.String() != tt.out {
				t.Fatalf("output mismatch\n----\n%s\n----\n%s\n",
					outbuf.String(), tt.out,
				)
			}
		})
	}
